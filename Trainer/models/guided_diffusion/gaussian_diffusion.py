...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]: # THIS
        ...
        elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]: # THIS
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            # i_list = [0, 1, 2, 3, 4, 100, 250, 500, 999]
            i_exceptions = [0, 1, 2]
            for i in indices:
                ...
                with torch.no_grad():
                    if img.shape != (imarr.shape[0], 3, 224, 224):
                        img = torch.cat(
                            (org_MRI, img), dim=1
                        )  # in every step, make sure to concatenate the original image to the sample
                    ...
                    # if i in i_list:
                    #  print('sampling step out ', i, np.min(np.asarray(img.detach().cpu())), np.max(np.asarray(img.detach().cpu())))
    ...
    def ddim_sample_loop_interpolation(
        ...
        assert isinstance(shape, (tuple, list))
        b = shape[0]
        t = torch.randint(499, 500, (b,), device=device).long().to(device)

        img1 = torch.tensor(img1).to(device)
        img2 = torch.tensor(img2).to(device)

        noise = torch.randn_like(img1).to(device)
    ...
    def ddim_sample_loop(
        ...
        assert isinstance(shape, (tuple, list))
        b = shape[0]
        t = torch.randint(99, 100, (b,), device=device).long().to(device)
    ...
    def training_losses_segmentation(
        ...
        x_t[:, -1:, ...] = res_t.float()  # replace last channel by noisy GT # ( void + mask + noisy_GT )

        terms = {}

        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # x_t - (16, 3, 224, 224); out - (16, 2, 224, 224)
            print('model_output', model_output.shape) #(mu, var)
            ...
            if self.model_var_type in [
                ...
                B, C = x_t.shape[:2]
                C = 1
                assert model_output.shape == (B, C * 2, *x_t.shape[2:]) # (16, 2, 224, 224): (mu, var)
                model_output, model_var_values = torch.split(model_output, C, dim=1)
            ...
            target = {
                ...
                ModelMeanType.EPSILON: noise, # THIS # the GT noise
            }[self.model_mean_type]
            terms["mse"] = mean_flat((target - model_output) ** 2) # noise MSE
            if "vb" in terms:
