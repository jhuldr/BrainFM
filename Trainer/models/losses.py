...
class SmoothnessLoss(nn.Module):
    def __init__(self, mode = 'l2'):
        ...
        if mode == 'l1':
            ...
            raise ValueError('Not supported loss_func for SmoothnessLoss:', mode)
    ...
    def forward(self, input):
        dx, dy, dz = self.gradient(input)  
        if self.mode == 'l1':
            return (abs(dx) + abs(dy) + abs(dz)).mean()
        elif self.mode == 'l2':
            return (dx ** 2 + dy ** 2 + dz ** 2).mean()
        else:
            raise NotImplementedError('Not supported loss mode for SmoothnessLoss:', self.mode)
...
class HessianLoss(nn.Module):
    ...
    def gradient(self, x): # gradient_c
        # x: (b, c, s, r, c) -->  dx, dy, dz: (b, c, s, r, c) 
        back = F.pad(x, [0, 1, 0, 0, 0, 0])[:, :, :, :, 1:] 
        right = F.pad(x, [0, 0, 0, 1, 0, 0])[:, :, :, 1:, :] 
        bottom = F.pad(x, [0, 0, 0, 0, 0, 1])[:, :, 1:, :, :]
    ...
    def forward(self, input): # det of the Hessian
        dx, dy, dz = self.gradient(input)  
        ddxx, ddxy, ddxz = self.gradient(dx)  
        ddxy, ddyy, ddyz = self.gradient(dy)  
        ddxz, ddyz, ddzz = self.gradient(dz)  
        det_hessian = ddxx * (ddyy * ddzz - ddyz ** 2) - ddxy * (ddxy * ddzz - ddxz * ddyz) + ddxz * (ddxy * ddyz - ddxz * ddyy)
        if self.mode == 'l1':
            return abs(det_hessian).sum()
        elif self.mode == 'l2':
            return (det_hessian ** 2).sum()
        else:
            raise NotImplementedError('Not supported loss mode for SmoothnessLoss:', self.mode)
